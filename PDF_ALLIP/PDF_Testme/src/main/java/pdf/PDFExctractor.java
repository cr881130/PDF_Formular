...
public class PDFExctractor implements PDFConstants
{
    ...
    public static int readFiles(final String names) throws Exception {
        int count = 0;
        final StringTokenizer st = new StringTokenizer(names, "\\\\\\");
        for (String name = st.nextToken(); name != null; name = st.nextToken()) {
            final boolean readFile = readFile(name);
            if (readFile) {
                ++count;
            }
        }
